...
[![Go Version](https://img.shields.io/badge/Go-1.24.4%2B-blue.svg)](https://go.dev/)
...
`bip38cli` is a CLI-first tool for working with BIP38-encrypted Bitcoin private keys. It focuses on interactive, scriptable workflows and keeps secrets out of shell history by prompting for passphrases securely.
...
- **Go 1.24.4+** — required to build from source; [download](https://go.dev/dl/)
- **golangci-lint v2** — required for `make lint`; install with `curl -sSfL https://raw.githubusercontent.com/golangci/golangci-lint/HEAD/install.sh | sh -s -- -b $(go env GOPATH)/bin v2.1.6`
...
```bash
# Prefer interactive prompt (avoids WIF in shell history)
bip38cli encrypt

# Or pass WIF as an argument (less safe on shared machines)
bip38cli encrypt KwYgW8gcxj1JWJXhPSu4Fqwzfhp5Yfi42mdYmMa4XqK7NJxXUSK7
# Hidden prompts ask for passphrase twice

# Force uncompressed format (default keeps the WIF's own compression)
bip38cli encrypt --uncompressed 5HueCGU8rMjxEXxiPuD5BDku4MkFqeZyd4dZ1jvhTVqvbTLvyTJ
...
```
bip38cli/                     # repository root (infrastructure)
├── go.mod / go.sum
├── Makefile
...
└── bip38cli/                 # all Go source
    ├── cmd/bip38cli/         # CLI entry point
    └── internal/
        ├── bip38/            # BIP38 domain logic and tests
        ├── cli/              # Cobra commands and UX flows
        ├── errors/
        ├── logger/
        └── metrics/
```
...
make uninstall  # Uninstall from $HOME/.local/bin
make version    # Show version information
make clean      # Remove build artifacts
```

The Go module lives at `github.com/carlosrabelo/bip38cli`. Tests for the BIP38 domain perform real scrypt key derivations and can take a few seconds.
