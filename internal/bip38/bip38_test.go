...
func TestEncryptDecryptRoundtrip(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Set compression as needed
			originalWIF.CompressPubKey = tt.compressed

			// Encrypt the key
			encryptedKey, err := EncryptKey(originalWIF, tt.passphrase)
			if err != nil {
				t.Fatalf("Failed to encrypt key: %v", err)
			}

			// Verify it's in BIP38 format
			if !IsBIP38Format(encryptedKey) {
				t.Errorf("Encrypted key is not in valid BIP38 format: %s", encryptedKey)
			}

			// Decrypt the key
			decryptedWIF, err := DecryptKey(encryptedKey, tt.passphrase)
			if err != nil {
...
func TestDecryptInvalidKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := DecryptKey(tt.encryptedKey, tt.passphrase)
			if err == nil {
