...
// TestEncryptDecryptProperty verifies the invariant:
//
//	DecryptKey(EncryptKey(wif, p), p).String() == wif.String()
//
// for randomly generated keys and passphrases. This is a lightweight
// property-based test that does not require an external PBT library.
		...
		for _, net := range networks {
			for _, compressed := range compressions {
				for i := 0; i < iterations; i++ {
					wif, err := GenerateWIF(net, compressed)
					if err != nil {
						t.Fatalf("GenerateWIF failed: %v", err)
					}

					encrypted, err := EncryptKey(wif, []byte(passphrase))
					if err != nil {
						t.Fatalf("EncryptKey failed (net=%s compressed=%v): %v", net.Name, compressed, err)
					}

					decrypted, err := DecryptKey(encrypted, []byte(passphrase))
					if err != nil {
						t.Fatalf("DecryptKey failed (net=%s compressed=%v passphrase=%q): %v",
							net.Name, compressed, passphrase, err)
					}

					if wif.String() != decrypted.String() {
						t.Errorf("roundtrip mismatch (net=%s compressed=%v):\n  original:  %s\n  decrypted: %s",
							net.Name, compressed, wif.String(), decrypted.String())
					}

					if wif.CompressPubKey != decrypted.CompressPubKey {
						t.Errorf("compression flag mismatch (net=%s): want %v got %v",
							net.Name, wif.CompressPubKey, decrypted.CompressPubKey)
					}
				}
			}
		}
