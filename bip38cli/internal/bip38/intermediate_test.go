...
func TestGenerateIntermediateCode(t *testing.T) {
	tests := []struct {
		...
		// boundary values — always run, not gated by testing.Short()
		{
			name:       "lot=0 seq=0 boundary",
			passphrase: "test",
			lot:        uint32Ptr(0),
			seq:        uint32Ptr(0),
			wantErr:    false,
		...
		{
			name:       "lot=max seq=max boundary",
			passphrase: "test",
			lot:        uint32Ptr(1048575),
			seq:        uint32Ptr(4095),
			wantErr:    false,
		},
		{
			name:       "lot=0 seq=max boundary",
			passphrase: "test",
			lot:        uint32Ptr(0),
			seq:        uint32Ptr(4095),
			wantErr:    false,
