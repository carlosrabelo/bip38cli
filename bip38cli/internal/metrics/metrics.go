...
// MarshalJSON implements json.Marshaler to avoid copying the mutex during serialization.
func (m *Metrics) MarshalJSON() ([]byte, error) {
	return json.Marshal(map[string]any{
		"encrypt_count":               m.EncryptCount,
		"decrypt_count":               m.DecryptCount,
		"intermediate_count":          m.IntermediateCount,
		"encrypt_duration_total":      m.EncryptDuration,
		"decrypt_duration_total":      m.DecryptDuration,
		"intermediate_duration_total": m.IntermediateDuration,
		"encrypt_errors":              m.EncryptErrors,
		"decrypt_errors":              m.DecryptErrors,
		"intermediate_errors":         m.IntermediateErrors,
		"average_encrypt_time":        m.AverageEncryptTime,
		"average_decrypt_time":        m.AverageDecryptTime,
		"average_intermediate_time":   m.AverageIntermediateTime,
...
var (
	// defaultMetrics is the global singleton, guarded by defaultMu so Reset can replace it.
	defaultMetrics *Metrics
	defaultMu      sync.Mutex
)

// GetMetrics returns the global metrics instance, creating it on first call.
func GetMetrics() *Metrics {
	defaultMu.Lock()
	defer defaultMu.Unlock()
	if defaultMetrics == nil {
		defaultMetrics = &Metrics{StartTime: time.Now()}
	}
...
// Reset resets all fields of this Metrics instance to zero and updates StartTime.
// When called on the global singleton it also replaces the singleton pointer so
// that GetMetrics() returns a fresh instance on the next call.
func (m *Metrics) Reset() {
	...
	// If this is the global singleton, replace it so GetMetrics() creates a new one.
	defaultMu.Lock()
	if defaultMetrics == m {
		defaultMetrics = nil
	}
...
// Stop stops the timer and records the metric if context hasn't been canceled
func (t *ContextTimer) Stop(success bool) {
	select {
	case <-t.ctx.Done():
		// Context was canceled, don't record the metric
		return
	default:
...
// Reset replaces the global metrics singleton with a fresh instance.
func Reset() {
	GetMetrics().Reset()
}
