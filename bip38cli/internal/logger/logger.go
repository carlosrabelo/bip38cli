// Package logger provides a configured logrus logger for the application.
package logger
...
// If debug is false, logs are formatted as JSON and warn level is used.
// Log output always goes to stderr so command results on stdout stay clean.
func Init(debug bool) {
	defaultLogger = logrus.New()

	// Set output to stderr so command results on stdout stay machine-parseable
