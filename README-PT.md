...
- **Go 1.24.4+** — necessário para compilar a partir do código; [download](https://go.dev/dl/)
- **golangci-lint v2** — necessário para `make lint`; instale com `curl -sSfL https://raw.githubusercontent.com/golangci/golangci-lint/HEAD/install.sh | sh -s -- -b $(go env GOPATH)/bin v2.1.6`
...
```bash
# Prefira prompt interativo (evita WIF no histórico do shell)
bip38cli encrypt

# Ou passe o WIF como argumento (menos seguro em máquinas compartilhadas)
bip38cli encrypt KwYgW8gcxj1JWJXhPSu4Fqwzfhp5Yfi42mdYmMa4XqK7NJxXUSK7
# O terminal solicita a senha duas vezes

# Forçar formato não comprimido (o padrão mantém a compressão do próprio WIF)
...
```
bip38cli/                     # raiz do repositório (infraestrutura)
├── go.mod / go.sum
├── Makefile
├── .make/                    # scripts de build, test e install
├── bin/                      # binários compilados (ignorados, exceto .gitkeep)
├── docs/                     # tutoriais
├── README.md / README-PT.md
└── bip38cli/                 # todo o código Go
    ├── cmd/bip38cli/         # ponto de entrada da CLI
    └── internal/
        ├── bip38/            # lógica de domínio BIP38 e testes
        ├── cli/              # comandos Cobra e fluxos de UX
        ├── errors/
        ├── logger/
        └── metrics/
```
