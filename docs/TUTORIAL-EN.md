...
- **Go 1.24.4+** (only required if you build from source); [download](https://go.dev/dl/)
...
# Uses the intermediate code to generate an EC-multiply encrypted key
bip38cli intermediate encrypt passphraseXXX...
# Saves and sends to Person A:
# - encrypted key (6P...)
# - confirmation code (cfrm38...)
...
### Important note about scripting passphrases

`bip38cli` reads passphrases using hidden terminal input (`ReadPassword`). This is intentional: it avoids leaking secrets via shell history, process lists, or plain stdin. As a result, **you generally cannot automate passphrase entry by piping**.

For automation, prefer:

- Running `bip38cli` interactively and capturing output via `--output-format json`
- Using a dedicated terminal session on a trusted machine
- External tooling such as `expect` (high risk; only when you fully understand the trade-offs)
...
for i in "${!KEYS[@]}"; do
    ...
    # Encrypt key (interactive passphrase prompt)
    RESULT=$(bip38cli encrypt --output-format json "${KEYS[i]}")

    if [[ $RESULT == *"\"encrypted_key\""* ]]; then
        echo "Key $((i+1)): $RESULT" >> "$OUTPUT"
        echo "✓ Key $((i+1)) encrypted successfully (JSON)"
    else
...
while IFS= read -r line; do
    if [[ $line == *":"* ]]; then
        ...
        # Try to decrypt
        RESULT=$(bip38cli decrypt --output-format json "$ENCRYPTED_KEY" 2>/dev/null)

        if [[ $RESULT == *"\"private_key\""* ]]; then
            echo "✓ $NUMBER: OK"
        else
            echo "✗ $NUMBER: ERROR"
        fi
