...
- **Go 1.24.4+** (somente se você for compilar a partir do código); [download](https://go.dev/dl/)
...
Para instalar no seu PATH de usuário:
...
# Usa o código intermediário para gerar uma chave criptografada via EC-multiply
bip38cli intermediate encrypt passphraseXXX...
# Salva e envia para Pessoa A:
# - chave criptografada (6P...)
# - código de confirmação (cfrm38...)
...
### Nota importante sobre automação de senhas

O `bip38cli` lê senhas usando entrada oculta no terminal (`ReadPassword`). Isso é proposital: evita vazar segredos via histórico do shell, lista de processos ou stdin em texto puro. Por isso, **em geral não dá para automatizar a digitação da senha com pipe**.

Para automação, prefira:

- Rodar o `bip38cli` de forma interativa e capturar saída via `--output-format json`
- Usar uma sessão de terminal dedicada em uma máquina confiável
- Ferramentas externas como `expect` (alto risco; use apenas entendendo bem os trade-offs)
...
for i in "${!CHAVES[@]}"; do
    ...
    # Criptografa a chave (senha solicitada de forma interativa)
    RESULTADO=$(bip38cli encrypt --output-format json "${CHAVES[i]}")

    if [[ $RESULTADO == *"\"encrypted_key\""* ]]; then
        echo "Chave $((i+1)): $RESULTADO" >> "$OUTPUT"
        echo "✓ Chave $((i+1)) criptografada com sucesso (JSON)"
    else
...
while IFS= read -r linha; do
    if [[ $linha == *":"* ]]; then
        ...
        RESULTADO=$(bip38cli decrypt --output-format json "$CHAVE_CRIPTO" 2>/dev/null)

        if [[ $RESULTADO == *"\"private_key\""* ]]; then
            echo "✓ $NUMERO: OK"
        else
            echo "✗ $NUMERO: ERRO"
        fi
